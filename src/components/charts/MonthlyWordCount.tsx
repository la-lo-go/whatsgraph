...
export function MonthlyWordCount({ messages }: MonthlyWordCountProps) {
	...
	const chartData = useMemo(() => {
		...
		for (const senderMessages of messages) {
			for (const message of senderMessages.messages) {
		...
		let processedData = Object.entries(monthlyData).map(
			([monthYear, senderCounts]) => ({
				monthYear,
				...senderCounts,
			}),
		);
		...
		if (showAllMonths) {
			...
			processedData = allMonths.map((monthYear) => {
				return monthlyData[monthYear]
					...
					: {
							monthYear,
							...Object.fromEntries(messages.map((m) => [m.sender_slug, 0])),
						};
			});
		}
		...
		<Card>
			<CardHeader className="flex items-center gap-2 space-y-0 border-b sm:flex-row">
				<div className="grid flex-1 gap-1 text-center sm:text-left">
					...
					<CardDescription>
						Total words written by each sender per month
					</CardDescription>
				</div>
						...
						<BarChart data={chartData}>
								...
								content={
									<ChartTooltipContent
										labelFormatter={(label) => {
											...
											return `${new Date(Number.parseInt(year), Number.parseInt(month) - 1).toLocaleString("default", { month: "long", year: "numeric" })}`;
										}}
									/>
								}
							/>
							...
							{senders.map((sender, index) => (
								<Bar
									...
									radius={
										index === senders.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]
									}
								/>
							))}
						</BarChart>
