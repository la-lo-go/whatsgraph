...
export function ParseWhatsAppMessages(input: string): WhatsAppMessages[] {
	...
	const messageRegex =
		/^(\d{1,2}\/\d{1,2}\/\d{2,4}),\s(\d{1,2}:\d{2})(?:\s-\s|\s–\s)(.*?):\s(.*)$/gm;

	for (const match of input.matchAll(messageRegex)) {
...
function formatDate(date: string, time: string): Date | null {
	...
	const formattedDate = new Date(
		`${yyyy}-${month.padStart(2, "0")}-${day.padStart(2, "0")}T${hours.padStart(2, "0")}:${minutes}:00`,
	);
...
function groupMessagesBySender(messages: Message[]): WhatsAppMessages[] {
	...
	for (const message of messages) {
		if (!groupedMessages.has(message.sender)) {
			...
			groupedMessages.set(message.sender, {
				sender: message.sender,
				sender_slug: slugged_sender,
				messages: [],
			});
		}
